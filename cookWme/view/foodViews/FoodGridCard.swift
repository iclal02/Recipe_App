...
struct FoodGridCard: View {
    ...
    // view başlatılırken isFavorite durumunu yemeğin mevcut durumundan alıyoruz
    init(food: Food) {
        self.food = food
...
func etiketButtons() -> [ActionSheet.Button] {
    ...
    if let user = authService.currentUser {
        for etiket in user.favoriEtiketleri {
            buttons.append(
                .default(Text(etiket.name)) {
                    authService.addFoodToEtiket(food: food, etiket: etiket)
                }
            )
        }
    }
